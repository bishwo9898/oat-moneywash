...
RegisterServerEvent('oat-moneywash:server:getmoney')
AddEventHandler('oat-moneywash:server:getmoney', function()
    local src = source
	local total_worth = 0
...
RegisterServerEvent('oat-moneywash:server:checkmoney')
AddEventHandler('oat-moneywash:server:checkmoney', function()
    local src = source
            ...
            if markedbills ~= nil then
                if ItemList[Player.PlayerData.items[k].name] ~= nil then 
                    if Player.PlayerData.items[k].name == "markedbills" and Player.PlayerData.items[k].amount >= 1 then 
					    ...
					    TriggerClientEvent("oat-moneywash:client:WashProggress", src)
                        break
                    else
                        TriggerClientEvent('QBCore:Notify', src, "You do not have marked money", 'error')
                        break
